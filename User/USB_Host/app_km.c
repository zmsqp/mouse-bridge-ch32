        ...
        else if( Com_Buf[ i + 1 ] == DEF_DECR_INTERFACE )
        {
            ...
            if( ( (PUSB_ITF_DESCR)( &Com_Buf[ i ] ) )->bInterfaceClass == 0x03 )
            {
                    ...
                    ( (PUSB_ITF_DESCR)( &Com_Buf[ i ] ) )->bInterfaceProtocol <= 2 )
                {
                    ...
                    {
                        /* 罗技等接收器常用 Protocol=0，稍后由报告描述符判定 */
...
void KM_AnalyzeHidReportDesc( uint8_t index, uint8_t intf_num )
{
    ...
    /* Usage Page(Generic Desktop), Usage(Mouse) — 罗技等 Protocol=0 的设备靠此识别 */
    if( ( Com_Buf[ 0 ] == 0x05 ) && ( Com_Buf[ 1 ] == 0x01 ) &&
        ( Com_Buf[ 2 ] == 0x09 ) && ( Com_Buf[ 3 ] == 0x02 ) )
    {
        HostCtl[ index ].Interface[ intf_num ].Type = DEC_MOUSE;

        for( j = 0; j + 1 < HostCtl[ index ].Interface[ intf_num ].HidDescLen; j++ )
        {
            if( Com_Buf[ j ] == 0x85 )
            {
                HostCtl[ index ].Interface[ intf_num ].IDFlag = 1;
                HostCtl[ index ].Interface[ intf_num ].ReportID = Com_Buf[ j + 1 ];
                break;
            }
        }
        return;
...
GETREP_START:
            ...
            if( s == ERR_SUCCESS )
            {
                ...
                if( HostCtl[ index ].Interface[ num ].Type == DEC_MOUSE )
                {
                    MouseBridge_SetReportDescriptor( Com_Buf, HostCtl[ index ].Interface[ num ].HidDescLen );
                    if( HostCtl[ index ].Interface[ num ].InEndpNum > 0 )
                    {
                        MouseBridge_SetEndpointParams(
        ...
        if( s == ERR_SUCCESS )
        {
            if( RootHubDev.bType == USB_DEV_CLASS_HID ) // Further enumerate it if this device is a HID device
            {
                ...
                if( s == ERR_SUCCESS )
                {
                    ...
                    MouseBridge_CloneDeviceDescriptor( DevDesc_Buf );
                    BridgeDebug_LogHostEnum(1, 0);
                ...
                for( in_num = 0; in_num < HostCtl[ index ].Interface[ intf_num ].InEndpNum; in_num++ )
                {
                    ...
                    if( HostCtl[ index ].Interface[ intf_num ].InEndpTimeCount[ in_num ] >= HostCtl[ index ].Interface[ intf_num ].InEndpInterval[ in_num ] )
                    {
                        ...
                        if( s == ERR_SUCCESS )
                        {
                            ...
                            {
                                MouseBridge_ForwardReport( Com_Buf, len );
                            }
                        }
                        else if( s == ERR_USB_DISCON )
                        {
